 ...
 * </p>
 * <p>
 * The <tt>options</tt> parameter, which appears in a few places, is
 * used to pass several pieces of information to the encoder. In the
 * "higher level" methods such as encodeBytes( bytes, options ) the
 * options parameter can be used to indicate such things as first
 * gzipping the bytes before encoding them, not inserting linefeeds
 * (though that breaks strict Base64 compatibility), and encoding
 * using the URL-safe and Ordered dialects.
 * </p>
 * <p>
 ...
 * <p>
 * to compress the data before encoding it and then making the output
 ...
 * less memory than encoding/decoding an entire file into memory
 * before writing.</li>
 * <li>v2.2.1 - Fixed bug using URL_SAFE and ORDERED encodings. Fixed
 * bug when using very small files (~< 40 bytes).</li>
 * <li>v2.2 - Added some helper methods for encoding/decoding directly
 * from one file to the next. Also added a main() method to support
 * command line encoding/decoding from one file to the next. Also
 * added these Base64 dialects:
 * <ol>
 * <li>The default is RFC3548 format.</li>
 * <li>Calling Base64.setFormat(Base64.BASE64_FORMAT.URLSAFE_FORMAT)
 * generates URL and file name friendly format as described in Section
 * 4 of RFC3548. http://www.faqs.org/rfcs/rfc3548.html</li>
 * <li>Calling Base64.setFormat(Base64.BASE64_FORMAT.ORDERED_FORMAT)
 * generates URL and file name friendly format that preserves lexical
 ...
 * <li>v2.1 - Cleaned up javadoc comments and unused variables and
 * methods. Added some convenience methods for reading and writing to
 * and from files.</li>
 * <li>v2.0.2 - Now specifies UTF-8 encoding in places where the code
 * fails on systems with other encodings (like EBCDIC).</li>
 * <li>v2.0.1 - Fixed an error when decoding a single byte, that is,
 * when the encoded data was a single byte.</li>
 * <li>v2.0 - I got rid of methods that used booleans to set options.
 * Now everything is more consolidated and cleaner. The code now
 * detects when data that's being decoded is gzip-compressed and will
 * decompress it automatically. Generally things are cleaner. You'll
 * probably have to change some method calls that you were making to
 * support the new options format (<tt>int</tt>s that you "OR"
 * together).</li>
 * <li>v1.5.1 - Fixed bug when decompressing and decoding to a byte[]
 * using <tt>decode( String s, boolean gzipCompressed )</tt>. Added
 * the ability to "suspend" encoding in the Output Stream so you can
 * turn on and off the encoding if you need to embed base64 data in an
 * otherwise "normal" stream (like an XML file).</li>
 * <li>v1.5 - Output stream pases on flush() command but doesn't do
 * anything itself. This helps when using GZIP streams. Added the
 ...
 * in input stream where last buffer being read, if not completely
 ...
 * plenty of well-wishing instead! Please visit <a
 * href="http://iharder.net/base64">http://iharder.net/base64</a>
...
public class Base64
{
	...
	/**
	 * Encode using Base64-like encoding that is URL- and Filename-safe
	 * as described in Section 4 of RFC3548: <a
	 * href="http://www.faqs.org/rfcs/rfc3548.html"
	 * >http://www.faqs.org/rfcs/rfc3548.html</a>. It is important to
	 * note that data encoded this way is <em>not</em> officially valid
	 * Base64, or at the very least should not be called Base64 without
	 * also specifying that is was encoded using the URL- and
	...
	// private final static byte BAD_ENCODING = -9; // Indicates error
	// in encoding
	...
	/*
	 * Host platform me be something funny like EBCDIC, so we hardcode
	 * these values.
	 */
	...
	/**
	 * Translates a Base64 value to either its 6-bit reconstruction
	 * value or a negative number indicating some other meaning.
	 **/
	private final static byte[] _STANDARD_DECODABET = { -9, -9, -9, -9, -9, -9, -9, -9, -9, // Decimal
			 ...
			 * 139 -9,-9,-9,-9,-9,-9,-9,-9,-9,-9,-9,-9,-9, // Decimal 140
			 * - 152 -9,-9,-9,-9,-9,-9,-9,-9,-9,-9,-9,-9,-9, // Decimal
			 * 153 - 165 -9,-9,-9,-9,-9,-9,-9,-9,-9,-9,-9,-9,-9, //
			 * Decimal 166 - 178 -9,-9,-9,-9,-9,-9,-9,-9,-9,-9,-9,-9,-9,
			 * // Decimal 179 - 191
			 * -9,-9,-9,-9,-9,-9,-9,-9,-9,-9,-9,-9,-9, // Decimal 192 -
			 * 204 -9,-9,-9,-9,-9,-9,-9,-9,-9,-9,-9,-9,-9, // Decimal 205
			 * - 217 -9,-9,-9,-9,-9,-9,-9,-9,-9,-9,-9,-9,-9, // Decimal
			 * 218 - 230 -9,-9,-9,-9,-9,-9,-9,-9,-9,-9,-9,-9,-9, //
			 * Decimal 231 - 243 -9,-9,-9,-9,-9,-9,-9,-9,-9,-9,-9,-9 //
			 * Decimal 244 - 255
			 */
			};
	...
	/**
	 * Used in the URL- and Filename-safe dialect described in Section
	 * 4 of RFC3548: <a
	 * href="http://www.faqs.org/rfcs/rfc3548.html">http
	 * ://www.faqs.org/rfcs/rfc3548.html</a>. Notice that the last two
	 * bytes become "hyphen" and "underscore" instead of "plus" and
	 * "slash."
	 */
	...
	/**
	 * I don't get the point of this technique, but it is described
	 * here: <a
	 * href="http://www.faqs.org/qa/rfcc-1940.html">http://www.
	 * faqs.org/qa/rfcc-1940.html</a>.
	 */
	 ...
	 * the options specified. It's possible, though silly, to specify
	 * ORDERED and URLSAFE in which case one of them will be picked,
	 ...
	 * the options specified. It's possible, though silly, to specify
	 * ORDERED and URL_SAFE in which case one of them will be picked,
	 * though there is no guarantee as to which one will be picked.
	 */
	...
	/**
	 * Encodes or decodes two files from the command line; <strong>feel
	 * free to delete this method (in fact you probably should) if
	 ...
	 * <var>threeBytes</var> and returns a four-byte array in Base64
	 * notation. The actual number of significant bytes in your array
	 * is given by <var>numSigBytes</var>. The array
	 * <var>threeBytes</var> needs only be as big as
	...
	/**
	 * <p>
	 * Encodes up to three bytes of the array <var>source</var> and
	 * writes the resulting four Base64 bytes to
	 * <var>destination</var>. The source and destination arrays can be
	 * manipulated anywhere along their length by specifying
	 * <var>srcOffset</var> and <var>destOffset</var>. This method does
	 * not check to make sure your arrays are large enough to
	 * accomodate <var>srcOffset</var> + 3 for the <var>source</var>
	 * array or <var>destOffset</var> + 4 for the
	 * <var>destination</var> array. The actual number of significant
	 * bytes in your array is given by <var>numSigBytes</var>.
	 * </p>
	 * <p>
	 * This is the lowest level of the encoding methods with all
	 * possible parameters.
	 * </p>
	 * 
	 * @param source
	...
	/**
	 * Serializes an object and returns the Base64-encoded version of
	 * that serialized object. If the object cannot be serialized or
	...
	public static byte[] decode( byte[] source, int off, int len, int options )
	{
		...
		{
			sbiCrop = ( byte ) ( source[ i ] & 0x7f ); // Only the low
																		// seven bits
			sbiDecode = DECODABET[ sbiCrop ];

			if( sbiDecode >= WHITE_SPACE_ENC ) // White space, Equals
															// sign or better
			{
	 ...
	 * within. Returns <tt>null</tt> if there was an error.
	 * 
	 * @param encodedObject
	...
	public static class InputStream extends java.io.FilterInputStream
	{
		...
		private byte[] alphabet; // Local copies to avoid extra method
											// calls
		...
		public InputStream( java.io.InputStream in, int options )
		{
			...
			this.position = -1;
			this.lineLength = 0;
			this.options = options; // Record for later, mostly to
	...
	public static class OutputStream extends java.io.FilterOutputStream
	{
		 ...
		 * to embed a piece of base640-encoded data in a stream.
		 * 
		 * @since 1.5.1
		 */
		 ...
		 * embed a piece of base640-encoded data in a stream.
		 * 
		 * @since 1.5.1
		 */
