...
public class Configuration {
	...
	/**
	 * The activity request flag used by the {@link ConfigActivity} (value = 1)
	 */
	...
	/**
	 * Extracts a {@link JSONObject} describing an object configuration tree
	 * 
	 * @param roots
		...
		for (int i = 0; i < roots.length; i++) {
			if (roots[i] != null) {
				...
				try {
					...
					Log.e(LOG_TAG, "Trouble extracting configuration from "
							+ roots[i], e);
				}
			}
		}
	...
	private static JSONObject extractConfig(Field f, Object o) throws Exception {
		try {
			...
			throw new Exception("Problem with " + o.getClass() + "."
					+ f.getName(), e);
		}
	...
	private static JSONObject extractConfig(Object o, Method m, Method n)
		...
		if (n == null) { // void or readonly
			...
			if (m.getReturnType() == void.class) { // action
				conf = new JSONObject();
				conf.put("type", "void");
			...
			if (setter.getReturnType() != void.class
				...
				throw new ConfigError("Setter method "
						+ setter.getDeclaringClass() + "." + setter.getName()
						+ " must have only one argument and a void return");
			}

			if (getter.getReturnType() == void.class
					|| getter.getParameterTypes().length != 0) {
				throw new ConfigError("Getter method "
			...
			if (getter.getReturnType() != setter.getParameterTypes()[0]) {
				throw new ConfigError("Return type of " + getter.getName()
						+ " must match argument type of " + setter.getName()
	 ...
	 * 
	 * @param m
	 * @return The name of the variable, or <code>null</code> if the method is
	 *         not {@link EncapVariable}
	 */
	...
	/**
	 * @param o
	 * @return {@link Variable} description, or <code>null</code> if there is
	 *         not one present
	 */
	...
	/**
	 * @param f
	 * @return {@link String} range, or <code>null</code> if there is not one
	 *         present
	 */
	...
	/**
	 * @param f
	 * @return numerical range, or <code>null</code> if there is not one present
	 */
	public static float[] getNumberRange(AccessibleObject f) {
		float[] nr = null;

		NumberRange r = f.getAnnotation(NumberRange.class);
		if (r != null) {
			nr = new float[r.value().length];

			for (int i = 0; i < nr.length; i++) {
				try {
					nr[i] = Float.parseFloat(r.value()[i]);
				} catch (NumberFormatException nfe) {
					throw new IllegalArgumentException(r.value()[i]
