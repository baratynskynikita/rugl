 ...
 * http://www.mesa3d.org/brianp/TR.html </a> . Original code Copyright
 * (C) 1997-2005 Brian Paul.
 * 
 * @author ryanm
 */
public class TileRenderer
{
	...
	private IntBuffer viewportSave = BufferUtils.createIntBuffer( 16 );
	 ...
	 */
	public void setTileBuffer( int format, int type, ByteBuffer image )
	{
		tileFormat = format;
		tileType = type;
		tileBuffer = image;
	 ...
	 *           final image
	 */
	public void setImageBuffer( int format, int type, ByteBuffer image )
	{
		imageFormat = format;
		imageType = type;
		imageBuffer = image;
	 ...
	 *           As in glOrtho
	 */
	public void trOrtho( double left, double right, double bottom, double top,
	 ...
	 *           As in glFrustrum
	 */
	public void trFrustum( double left, double right, double bottom, double top,
			double zNear, double zFar )
	{
		perspective = true;
		this.left = left;
	...
	public void beginTile()
	{
		if( currentTile <= 0 )
		{
			 ...
			 */
			viewportSave.clear();
			GL11.glGetInteger( GL11.GL_VIEWPORT, viewportSave );
		}
		...
		IntBuffer matrixMode = BufferUtils.createIntBuffer( 16 );
		GL11.glGetInteger( GL11.GL_MATRIX_MODE, matrixMode );
		...
		if( perspective )
		{
			GL11.glFrustum( l, r, b, t, near, far );
		}
		else
		{
		...
		GL11.glMatrixMode( matrixMode.get( 0 ) );
	}
	...
	public boolean endTile()
	{
		...
		IntBuffer previousPixelStoreValues = BufferUtils.createIntBuffer( 20 );
		previousPixelStoreValues.position( 0 );
		GL11.glGetInteger( GL11.GL_PACK_ROW_LENGTH, previousPixelStoreValues );
		previousPixelStoreValues.position( 1 );
		GL11.glGetInteger( GL11.GL_PACK_SKIP_ROWS, previousPixelStoreValues );
		previousPixelStoreValues.position( 2 );
		GL11.glGetInteger( GL11.GL_PACK_SKIP_PIXELS, previousPixelStoreValues );
		previousPixelStoreValues.position( 3 );
		GL11.glGetInteger( GL11.GL_PACK_ALIGNMENT, previousPixelStoreValues );
		...
		if( tileBuffer != null )
		{
			...
			GL11.glReadPixels( srcX, srcY, srcWidth, srcHeight, tileFormat, tileType,
					tileBuffer );
		}
		...
		if( imageBuffer != null )
		{
			...
			GL11.glPixelStorei( GL11.GL_PACK_ROW_LENGTH, imageSize.width );
			GL11.glPixelStorei( GL11.GL_PACK_SKIP_ROWS, destY );
			GL11.glPixelStorei( GL11.GL_PACK_SKIP_PIXELS, destX );
			GL11.glPixelStorei( GL11.GL_PACK_ALIGNMENT, 1 );

			/* read the tile into the final image */
			GL11.glReadPixels( srcX, srcY, srcWidth, srcHeight, imageFormat, imageType,
					imageBuffer );
		}

		/* restore previous glPixelStore values */
		GL11.glPixelStorei( GL11.GL_PACK_ROW_LENGTH, previousPixelStoreValues.get( 0 ) );
		GL11.glPixelStorei( GL11.GL_PACK_SKIP_ROWS, previousPixelStoreValues.get( 1 ) );
		GL11.glPixelStorei( GL11.GL_PACK_SKIP_PIXELS, previousPixelStoreValues.get( 2 ) );
		GL11.glPixelStorei( GL11.GL_PACK_ALIGNMENT, previousPixelStoreValues.get( 3 ) );
		...
		if( currentTile >= rows * columns )
		{
			...
			GL11.glViewport( viewportSave.get( 0 ), viewportSave.get( 1 ),
					viewportSave.get( 2 ), viewportSave.get( 3 ) );
