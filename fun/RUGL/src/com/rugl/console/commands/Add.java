						...
						if( conf != null )
						{
							...
							for( int i = 0; i < conf.getNames().length && !found; i++ )
							{
								if( conf.getNames()[ i ] instanceof String )
								{
									...
									if( ( conf.getType( v ) == int.class || conf.getType( v ) == float.class )
											&& v.compareToIgnoreCase( var ) == 0 )
									{
										var = v;
										found = true;
									}
							...
							if( found )
							{
								...
								Console
										.log( path + "/" + var + " set to " + conf.getValue( var ) );
							}
							else
							{
								Console.error( "Could not find numerical variable \"" + var
