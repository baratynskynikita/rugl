...
import com.rugl.geom.TexturedShapeBuilder;
...
public class TextLayout
{
			...
			float lineLength, int colour )
	{
		...
		TexturedShapeBuilder tsb = new TexturedShapeBuilder();
		...
		do
		{
			...
			{
				ColouredShape ts = font.buildTextShape( lineArray[ i ], colour );
			...
			float lineLength )
	{
		...
		do
		{
			...
			else
			{
				...
				if( next == prev )
				{ // we have no choice but to split a word
					next += increment;
				}
			}
