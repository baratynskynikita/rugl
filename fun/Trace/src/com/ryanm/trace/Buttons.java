...
public class Buttons
{
	...
	private static int[] keyCodes = new int[] { Keyboard.KEY_ESCAPE, Keyboard.KEY_RETURN,
	...
	private static TexturedShape[] textShapes = new TexturedShape[] { null, null, null,
			null, null, null };
	...
	public static void init()
	{
		if( buttons == null )
		{
			...
			ColouredShape escButton =
					new ColouredShape( ShapeUtil.innerQuad( escStart.x, escStart.y, escStart.x
							+ size, escStart.y + size, thickness, escPos.z ), Colour.white, null );
			mouseShapes[ 0 ] =
					ShapeUtil.filledQuad( escStart.x, escStart.y, escStart.x + size,
							escStart.y + size, 0 );

			Shape arrow =
					ShapeUtil.arrow( 0, 0, arrowSize.x, arrowSize.y, arrowShape.x,
							arrowShape.x );
			...
			Vector2f v =
					new Vector2f( 800 - 10 - size - cursorSep - size - cursorSep - size, 10
							+ cursorSep + size );
			ColouredShape lb =
					new ColouredShape( box
							.clone()
							.transform(
									MatrixUtils.rotateAround( Trig.PI / 2, size / 2, size / 2 ) )
							.translate( v.x, v.y, 1 ), Colour.white, null );
			mouseShapes[ 4 ].transform(
					MatrixUtils.rotateAround( Trig.PI / 2, size / 2, size / 2 ) ).translate(
					v.x, v.y, 1 );
			...
			ColouredShape db =
					new ColouredShape( box.clone()
							.transform( MatrixUtils.rotateAround( Trig.PI, size / 2, size / 2 ) )
							.translate( v.x, v.y, 1 ), Colour.white, null );
			mouseShapes[ 3 ].transform(
					MatrixUtils.rotateAround( Trig.PI, size / 2, size / 2 ) ).translate( v.x,
					v.y, 1 );

			v.y += 2 * ( size + cursorSep );
			ColouredShape ub =
					new ColouredShape( box.clone().translate( v.x, v.y, 1 ), Colour.white,
							null );
	...
	public static void advance( float delta )
	{
		if( buttons != null )
		{
			for( int i = 0; i < keyCodes.length; i++ )
			{
				...
				alpha =
						Math.max( alpha, textShapes[ i ] == null ? disabledAlpha
								: inactiveAlpha );
	...
	{
		Shape outline =
				ShapeUtil.outline( thickness, 1, retPos.x + retShape.x, retPos.y, retPos.x
						+ retShape.x, retPos.y + retShape.y, retPos.x, retPos.y + retShape.y,
						retPos.x, retPos.y + retSize.y, retPos.x + retSize.x, retPos.y
								+ retSize.y, retPos.x + retSize.x, retPos.y );

		Shape arrow =
				ShapeUtil.retArrow( retArrowPos.x, retArrowPos.y, retArrowSize.x,
						retArrowSize.y, retArrowShape.x, retArrowShape.y );
	...
	{
		Shape s =
				Tesselator.tesselate( new float[] { retPos.x + retShape.x, retPos.y,
						retPos.x + retShape.x, retPos.y + retShape.y, retPos.x,
						retPos.y + retShape.y, retPos.x, retPos.y + retSize.y,
						retPos.x + retSize.x, retPos.y + retSize.y, retPos.x + retSize.x,
						retPos.y }, 1 );
		return s;
	}
	 ...
	 * @param right
	 */
	public static void setText( String escape, String enter, String up, String down,
		...
		for( int i = 0; i < texts.length; i++ )
		{
			...
			if( changed )
			{
				...
				if( texts[ i ] != null )
				{
					...
					if( i == 1 )
					{// special case - want enter and esc texts to line up
