...
public abstract class Codec<P> {
	 ...
	 */
	public abstract P decode(String encoded, Class runtimeType)
			throws ParseException;
	...
	/**
	 * Maps from a type identifier to a codec that reports that it can handle
	 * that type
	 */
	private static Map<Class, Codec<? extends Object>> codecs = new HashMap<Class, Codec<? extends Object>>();
	static {
	 ...
	 * @return true if the codec is registered successfully, false otherwise
	 */
	public static boolean registerCodec(Codec<?> codec) {
		if (!codecs.containsKey(codec.getType())) {
			...
			System.err
					.println("Attempted to register duplicate codec for type \""
							+ codec.getType() + "\"");
			return false;
		}
	}
	...
	/**
	 * Finds the most suitable codec for a type. Searches through the type's
	 * superclass hierarchy for a matching codec
	 * 
	 * @param type
	 * @return The most specific codec possible, or null if none is found
	 */
