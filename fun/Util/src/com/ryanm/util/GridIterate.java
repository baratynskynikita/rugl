...
public class GridIterate
{
	...
	private final Vector2f start = new Vector2f(), end = new Vector2f(), p = new Vector2f(),
			q = new Vector2f();
	...
	public void next()
	{
		...
		{
			lastGridCoords.setX( lastGridCoords.getX() + lastGridExit.x );
			lastGridCoords.setY( lastGridCoords.getY() + lastGridExit.y );
		}
		...
		// clip to grid
		lastGridExit = clip( p, q, lastGridCoords.getX(), lastGridCoords.getY() );

		lastGridExit = dir.validate( lastGridExit );

		if( lastGridExit == Move.COMPLETE || lastGridCoords.getX() < min.x
				|| lastGridCoords.getY() < min.y || lastGridCoords.getX() > max.x
				|| lastGridCoords.getY() > max.y )
		{
			done = true;
		}
	}
